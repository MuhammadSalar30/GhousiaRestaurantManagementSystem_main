                    ...
                    <div class="swiper cart-swiper">
                        <div class="swiper-wrapper">
                            ...
                            <div class="swiper-slide">
                                <div class="w-full h-96 lg:h-[500px] flex items-center justify-center bg-gray-50 rounded-lg overflow-hidden">
...
@if(!empty($sizes))
                <div class="flex items-center gap-3 mb-4">
                    ...
                    @if(isset($sizes['half']))
                    <div>
                        <input type="radio" name="size_option" id="size-half" value="half" class="peer hidden" />
                        <label for="size-half" class="w-14 h-9 flex justify-center items-center cursor-pointer select-none rounded-full text-sm text-center bg-default-200 peer-checked:bg-primary peer-checked:text-white">Half</label>
                    </div>
                    @endif
                    @if(isset($sizes['full']))
                    <div>
                        <input type="radio" name="size_option" id="size-full" value="full" class="peer hidden" />
                        <label for="size-full" class="w-14 h-9 flex justify-center items-center cursor-pointer select-none rounded-full text-sm text-center bg-default-200 peer-checked:bg-primary peer-checked:text-white">Full</label>
                    </div>
                    @endif
                </div>
                ...
                <div class="mb-6">
                    <h4 class="font-semibold text-2xl text-default-900" id="product-price" data-sizes='@json($sizes)'>
                        ...
                        @else
                            PKR {{ number_format((float)($data->price ?? $data->min_price ?? 0), 0) }}
                        @endif
                    </h4>
                </div>
@endif
...
@foreach(($recommendations ?? []) as $rec)
            <div class="group border border-default-200 rounded-lg p-4 overflow-hidden hover:border-primary transition-all duration-300">
                <div class="relative rounded-lg overflow-hidden divide-y divide-default-200">
                    <div class="w-full h-52 mb-4 mx-auto bg-gray-50 rounded-lg overflow-hidden">
                        <img src="{{ $rec->image ? asset($rec->image) : '/images/dishes/burger.png' }}" class="w-full h-full object-cover group-hover:scale-105 transition-all">
                    </div>
                    <div class="pt-2">
                        ...
                        <div class="flex items-end justify-between mb-4">
                            <h4 class="font-semibold text-xl text-default-900">PKR {{ number_format((float)$rec->price, 2) }}</h4>
                        </div>
...
@section('script')
  ...
  <script>
    ...
    function addToCart() {
        ...
        console.log('Adding to cart - Quantity from input:', quantityInput.value, 'Parsed quantity:', quantity);
        ...
        .then(data => {
            ...
            if (data.success) {
                // Update all cart count elements with server response
    ...
    function updateCartCount(count) {
        ...
        // Show bottom cart bar if count > 0
        if (count > 0 && typeof showBottomCartBar === 'function') {
            showBottomCartBar();
        }
    }
    ...
    function toggleWishlist(itemId, el) {
        ...
        .catch(error => {
            ...
            showNotification('Please login to use wishlist feature', 'error');
        });
    }
